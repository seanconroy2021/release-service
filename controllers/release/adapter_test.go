...
var _ = Describe("Release adapter", Ordered, func() {
	...
	When("EnsureFinalPipelineIsProcessed is called", func() {
		...
		It("should mark the Final Pipeline Processing as Skipped if the ReleasePlan isn't found", func() {
			adapter.ctx = toolkit.GetMockedContext(ctx, []toolkit.MockData{
				{
					ContextKey: loader.ProcessingResourcesContextKey,
					Err:        fmt.Errorf("no ReleasePlan can be found"),
				},
			})
			adapter.release.MarkManagedPipelineProcessingSkipped()
	...
	When("EnsureReleaseFinalProcessingIsTracked is called", func() {
		var adapter *adapter
	...
	When("EnsureReleaseManagedProcessingIsTracked is called", func() {
		var adapter *adapter
	...
	When("EnsureReleaseProcessingResourcesAreCleanedUp is called", func() {
		...
		It("Should continure if the Release final processing has not finished", func() {
